...
export class Scene1Intro extends BaseScene {
  async enter() {
    ...
    const finishTyping = () => {
      ...
      isTyping = false;
      vnText.textContent = currentFullText;
    };
    ...
    const updateSpeaker = (speaker) => {
      ...
      const map = { me: '我', her: '她', system: '系统' };
      const label = map[speaker] ?? speaker;
      // 先清除颜色类
      [speakerLeft, speakerRight].forEach((el) => {
        el.classList.remove('speaker-me', 'speaker-her', 'speaker-system');
      });
      if (speaker === 'me') {
        speakerLeft.textContent = label;
        speakerLeft.classList.remove('hidden');
        speakerLeft.classList.add('speaker-me');
        speakerRight.classList.add('hidden');
      } else if (speaker === 'her') {
        speakerRight.textContent = label;
        speakerRight.classList.remove('hidden');
        speakerRight.classList.add('speaker-her');
        speakerLeft.classList.add('hidden');
      } else if (speaker === 'system') {
        speakerRight.textContent = label;
        speakerRight.classList.remove('hidden');
        speakerRight.classList.add('speaker-system');
        speakerLeft.classList.add('hidden');
      }
    };
    /**
     ...
     *  - 设置阶段完成回调
     */
    const appendLinesProgressively = (stage, done) => {
      lineQueue = stage && stage.lines ? [...stage.lines] : [];
      lineIndex = 0;
      stageDoneCallback = done;
      awaitingLine = false;
      vnText.textContent = '';
      renderNextLine();
    };
    ...
    const endStage = (stage) => {
      if (stage.end) {
        const nextScene = stage.end.next || 'exam';
        phaseMsg.textContent = '……';
        setTimeout(() => this.ctx.go('transition', { next: nextScene, style: 'flash12' }), 700);
        return true;
      }
      return false;
    };
    ...
    const handleChoice = (choice) => {
      ...
      if (choice.win === true) {
        ...
        if (vnBox) {
          const winText =
            (script.meta && script.meta.winText) ||
            '她终于相信了你。\n（这里可以写更走心的过渡文案）';
          const isCompleted =
            typeof localStorage !== 'undefined' &&
            localStorage.getItem &&
            localStorage.getItem('birthday_completed_mark') === 'true';
          const finalBtnHtml = isCompleted
            ? `<button class='btn-go-final' style='margin-left:.6rem;padding:.65rem 1.2rem;font-size:.95rem;border:none;border-radius:8px;background:#6ab04c;color:#fff;cursor:pointer;'>回到通关页面</button>`
            : '';
    ...
    const renderChoices = (stage) => {
      ...
      choicesBox.querySelectorAll('button').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          ...
          btn._handled = true;
          setTimeout(() => (btn._handled = false), 400);
     ...
     */
    const goStage = (id) => {
      const st = findStage(id);
      if (!st) return;
      this.currentStage = st; // 清空残留选项防止上一阶段按钮闪留
      const choicesBox = el.querySelector('.dynamic-choices');
      if (choicesBox) choicesBox.innerHTML = '';
      appendLinesProgressively(st, () => {
        if (endStage(st)) return;
        renderChoices(st);
      });
    };
    ...
    const advanceHandler = (e) => {
      ...
      // 避免点击选项按钮触发推进
      if (e && e.target && e.target.tagName === 'BUTTON' && e.target.closest('.dynamic-choices'))
        return;
