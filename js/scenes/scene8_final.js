...
export class Scene8Final extends BaseScene {
  ...
  async enter() {
    ...
    try {
      localStorage.setItem('birthday_completed_mark', 'true');
    } catch (e) {}
    el.querySelector('.replay').addEventListener('click', () => {
      // 点击重新开始旅程时清除通关印记并回到注册页
      try {
        localStorage.setItem('birthday_completed_mark', 'false');
      } catch (e) {}
      try {
        // 移除曾记录的进入终章事件，避免再次注册后被旧事件判定为极速通关
        if (achievements && Array.isArray(achievements.events)) {
          achievements.events = achievements.events.filter(
            (ev) => !(ev && ev.name === 'scene8:entered')
          );
        }
        // 撤销相关已解锁标记（如果已解锁），并持久化
        try {
          if (achievements && achievements.unlocked) {
            achievements.unlocked.delete('8-0');
            achievements.unlocked.delete('8-1');
          }
        } catch (e) {}
        try {
          if (achievements && typeof achievements._save === 'function') achievements._save();
        } catch (e) {}
        // 重置已注册时间戳，确保后续注册会写入新的时间点
        try {
          if (achievements) achievements._registeredAt = null;
        } catch (e) {}
      } catch (e) {
        console.warn('[Scene8] replay cleanup err', e);
      }
    ...
    try {
      const completed = (() => {
        try {
          return localStorage.getItem('birthday_completed_mark') === 'true';
        } catch (e) {
          return false;
        }
      })();
    ...
    // 记录进入终章（第八幕），供成就判定使用
    try {
      achievements.recordEvent('scene8:entered', { ts: Date.now() });
    } catch (e) {}
    // 绑定成就按钮
    try {
      const viewBtn = el.querySelector('.view-ach');
      if (viewBtn) {
        viewBtn.addEventListener('click', () => openAchievementsModal());
      }
    } catch (e) {}
  }
  async exit() {
    audioManager.stopBGM('7', { fadeOut: 600 });
  }
}
...
function openAchievementsModal() {
  ...
  items.forEach((it) => {
    ...
    if (it.unlocked) {
      body.innerHTML = `<div class='ach-title'>${escapeHtml(
        it.meta.title || it.id
      )}</div><div class='ach-desc'>${escapeHtml(it.meta.desc || '')}</div>`;
    } else {
      if (it.meta && it.meta.descriptionVisible) {
        body.innerHTML = `<div class='ach-title' style='color:#666'>${escapeHtml(
          it.meta.title || '未知成就'
        )}</div><div class='ach-desc' style='color:#888'>${escapeHtml(
          it.meta.desc || '暂无描述'
        )}</div>`;
      } else {
        ...
        placeholder.addEventListener('click', () => {
          try {
            ...
            // 记录以便在模态关闭时统一恢复
            try {
              persistRevealed.push({ placeholder, descDiv });
            } catch (e) {}
          } catch (e) {
            /* defensive */
          }
        ...
        try {
          for (const rec of persistRevealed) {
            try {
              ...
              const d = rec.descDiv;
              if (ph) {
                ph.dataset.revealed = '0';
                ph.classList.remove('revealed-temp');
                ph.textContent = '锁定的成就';
              }
...
function openJumpPanel(sceneInstance) {
  ...
  sceneIds.forEach((id, idx) => {
    ...
    b.addEventListener('click', () => {
      ...
      try {
        sceneInstance.ctx.go(id);
      } catch (e) {
        // 兜底：若路由名不同，可尝试使用 'register' 或 'timeline' 之类的保底跳转
        try {
          sceneInstance.ctx.go('register');
        } catch (e2) {}
      }
    });
    list.appendChild(b);
  });
  panel.querySelector('#jump-close').addEventListener('click', () => {
    try {
      overlay.remove();
    } catch (e) {}
  });
