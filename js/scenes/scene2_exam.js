 ...
 *
 * 目标与设计要点：
 * - 零挫败体验：无论答错或跳过都尽量给分，鼓励探索与陪伴感
 * - 数据驱动：题库由外部文件 `data/scene2_questions.json` 提供
 * - 稳定性：通过场景管理器与 DOM 清理避免重复渲染
 * - 可扩展：题型、计分、成就等在集中位置可扩展
 *
 * 主要规则：
 * - 得分：每题基础分 2 × 难度权重（easy=1, medium=2, hard=3）
 * - 提示：每题最多 3 次提示，第 3 次自动判定为通过并展示答案
 * - 彩蛋判定：全跳过 > 全自动提示 > 全错误宠溺
 * - 计时：按科目独立计时，进入科目启动 5 分钟倒计时
 *
 * 数据契约（question 对象）：
 * {
...
function extractUserAnswer(wrapper, q) {
  switch (q.type) {
    case 'fill':
      return wrapper.querySelector('.q-input')?.value.trim();
    case 'single_select': {
...
function getAnswerReveal(q) {
  ...
  if (q.type === 'multi_select') {
    const arr = Array.isArray(q.answerIndex)
      ? q.answerIndex
      : Array.isArray(q.answer)
      ? q.answer
      : [];
...
export class Scene2Exam extends BaseScene {
  ...
  async enter() {
    ...
    const stopSubjectTimer = () => {
      if (this._subjectTimer) {
        clearInterval(this._subjectTimer);
      ...
      this._betweenTimer = setInterval(() => {
        ...
        if (wait <= 0) {
          ...
          if (!nextSubject.done) {
            ...
            status.innerHTML = `<span class='ok between-hint'>${
    ...
    const stopBetweenTimer = () => {
      if (this._betweenTimer) {
        clearInterval(this._betweenTimer);
      ...
      tabsBox.innerHTML = ordered
        .filter((s) => {
          ...
          return (
            !locked ||
            s.done ||
            s.key === this._activeSubjectKey ||
            s.key === this._displayedSubjectKey
          );
        })
        .map((s) => {
          const activeCls = s.key === this._activeSubjectKey ? ' active' : '';
          const dispCls = s.key === this._displayedSubjectKey ? ' displayed' : '';
          return `<button class='tab${activeCls}${dispCls}' data-sub='${s.key}'>${s.title}${
            s.done ? '✔' : ''
          }</button>`;
        })
        .join('');
      tabsBox.querySelectorAll('.tab').forEach((tb) =>
    ...
    const renderSubject = (subject, startTimer = false) => {
      ...
      const typeLabel =
        nextQ.type === 'fill'
          ? '填空题'
          : nextQ.type === 'single_select'
          ? '单选题'
          : nextQ.type === 'multi_select'
          ? '多选题'
          : nextQ.type;
      wrapper.innerHTML = `
        ...
        <div class='progress-mini'>本科进度：${subject.questions.filter((q) => q.solved).length}/${
      ...
      hintBtn.addEventListener('click', () => {
        ...
        if (nextQ.hintCount <= 2) {
          ...
          const text =
            hintsArr[idx] ||
            (idx === 0 ? '暂无提示' : hintsArr[0] ? hintsArr[0] : '没有更多提示了');
      ...
      submitBtn.addEventListener('click', () => {
        ...
        if (correct) {
          ...
          const gained = POINTS_BY_TYPE[nextQ.type] || 2;
          addScoreToSubject(subject.key, gained);
          status.innerHTML = `<span class='ok'>${
            nextQ.correctMsg || '太棒啦！'
          }</span> <em>+${gained} 分</em>`;
          updateHUD();
          wrapper._locked = true;
          submitBtn.disabled = true;
    ...
    const renderSummary = () => {
      ...
      // 统计自动提示完成
      const autoHintCount = this.subjects.reduce(
        (acc, s) => acc + s.questions.filter((q) => q._autoHint).length,
        0
      );
      ...
      summaryEl.innerHTML = `
        <div class='summary-card'>
          ...
          <div class='subject-breakdown'>
            ${(this.subjects || [])
              .map(
                (s) => `<div class='sb-line'>${s.title}: ${this.subjectScores[s.key] || 0}</div>`
              )
              .join('')}
          </div>
          ${easterHTML}
