...
计分 / 宠溺 规则（最新版）：
- 正确：`得分 = 2 * 难度权重`（提示不再扣分）。
- 提示（多次机制）：一题最多 3 次——前两次显示提示文本，第 3 次直接送答案并判定满分通过。
- 宠溺跳过（“不会但我超可爱”）：`得分 = 2 * 难度权重`（无惩罚）。
- 错误宠溺通过：答错也判定完成并给“完整得分” `2 * 难度权重`，且展示正确答案。
...
2. 加一个“认真模式”开关（条件判断包裹跳过/错误逻辑 & 第三次提示不给答案）。
...
---
### 11. 纪念日注册小仪式（场景0）
为增加仪式感，在正式进入主线前新增“注册”页：
- 用户名：随便输入（不校验，可留空）。
- 密码：必须为在一起那天日期（格式 YYYYMMDD，当前内置：`20241007`）。
- 通过后写入 `localStorage.hasRegistered='1'`，下次自动跳过。
- 错误次数线索：
  - ≥3 次：显示线索一（年份 2024）。
  - ≥5 次：显示线索二（月份 10）。
  - ≥7 次：显示线索三（日期 07 + 最终组合）。
- “我忘了” 按钮：随时展开当前可见线索并给予鼓励话语。

_最近更新：加入注册仪式、提示三次自动送答案、提示不再扣分、错误/跳过/第三次提示均为满分宠溺路径。_
