...
**当前实现（已简化为“纯分支视觉小说”）**
- 背景音乐（BGM）自动播放。
- 开场：她问题 → 我因穿越条件反射握手 → 她惊讶质问。
- 所有“信任 / 安抚”数值与进度条机制已移除；不再存在阈值解锁、组合加成、自动跳转。
...
如需回退旧机制，可从 git 历史找回 2024-xx-xx 之前版本的 `scene1_intro.js` 与脚本结构。

**可选后续扩展（若未来想再增强）**
- 用 tags（已保留）记录玩家“陪伴风格”，后续场景引用。
- 添加轻量分支统计：记录路径用于终章引用。
- 增加“记忆证明失败”额外分支（当前留作简单循环返回）。
...
---
## 🧩 自定义常用入口
<!-- 旧：修改安抚按钮（已移除数值机制，直接通过脚本 JSON 的 choices 调整）。 -->
