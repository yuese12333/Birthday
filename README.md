...
### 5. 第五幕：心跳扫雷（Scene5Date）

**当前实现（扫雷玩法 + 外部化关卡）**

- 玩法：把第 5 幕改为扫雷（Minesweeper）。左键打开格子，右键标记旗帜，0 邻居会递归展开。
- 网格支持矩形：使用 `rows` 与 `cols`（JSON schema 迁移自旧 `gridSize`）。
- 外部关卡：`data/scene5_levels.json` 的 `levels[]` 数组，每个 level 推荐字段：

  - `id`、`title`、`tip`
  - `rows` (number)、`cols` (number)
  - `mineCounts`: `{ easy, medium, hard }`（编辑器只保存每档雷数，场景按难度随机布雷）
  - `disabled`: `[[r,c], ...]`（禁用格，不参与布雷与邻居计数，保持格位占位）

- 编辑器：`editor/scene5_levels_editor.py`（PyQt6）用于编辑 `rows`/`cols`、`mineCounts` 与 `disabled`；编辑器**不再创建或保存** `mines` 字段（随机布雷在运行时生成）。

- 难度与开始：界面提供 `easy`/`medium`/`hard` 三个按钮。必须显式选择难度后才会开始本局（随机布雷按对应 `mineCounts`）。

- 作弊与提示：内置“魂斗罗式”键序（Konami）可解锁提示按钮：

  - 键序：ArrowUp,ArrowUp,ArrowDown,ArrowDown,ArrowLeft,ArrowRight,ArrowLeft,ArrowRight
  - 解锁后出现 `提示` 按钮，点击会随机打开一个非雷、未打开且未禁用的安全格。
  - 触发作弊会上报事件 `scene5:cheat_unlocked`（用于屏蔽某些成就）。

- 传染机制（新玩法）：每次有效左键点击前有 10% 概率令被点击格子邻域（3×3）内的「一个合法非雷格」变为雷：

  - 不会作用于禁用、已标记、已打开或已为雷的格子。
  - 明确禁止把“中心格”（当前点击格）作为传染目标。
  - 每次点击最多传染一个格，每局最多发生 3 次传染。
  - 传染成功时会在左下角以 toast 弹出提示（文本："附近出现了新的雷……小心！"），并局部刷新已打开相邻格的数字显示与剩余雷数。

- 成就与事件（部分）

  - 场景会上报的事件（样例）：
    - `scene5:stepped_mine` — 玩家踩雷（payload 包含 level, difficulty, first）
    - `scene5:first_click_mine` — 本关第一次点击即踩雷
    - `scene5:cheat_unlocked` — 解锁提示按钮（作弊）
    - `scene5:completed` — 关卡完成（payload 包含 level, difficulty, final）
    - `scene5:infected_double` — 连续两次有效点击均触发传染（用于成就 5-5）

- 成就 5-5（病原体）：当玩家连续两次有效点击（两次相邻的左键点击）都触发了传染时，上报 `scene5:infected_double`，成就会检测该事件并解锁（在 `js/core/achievements.js` 中已注册 id=`5-5`，descriptionVisible: false）。实现细节：只有在 onLeftClick 中捕获 tryInfect 返回 true 的两次连续点击才会触发该事件。

- UI 与交互细节：
  - 网格渲染文件：`js/scenes/scene5_date.js`。
  - 剧情跳转：完成最后一关后页面会显示“下一关 / 进入下一幕”按钮；进入下一幕会使用项目通用的转场调用（`this.ctx.go('transition', { next: 'scene6', style: 'flash12' })` 或等价调用，保持与其它场景一致）。
  - 失败（踩雷）后会 reveal 全部雷，界面确保难度按钮与重开按钮恢复可用状态。

**实现/维护要点**

- 随机布雷会在 `startGame(difficulty)` 时生成，`this.mines` 仅为调试/展示用途（不是编辑器持久化字段）。
- `disabled` 格不会参与布雷与邻居计数，但保留格位占位，编辑器允许设置并标准化写入 JSON。
- 传染实现注意点：在 `tryInfect(clickR, clickC)` 内会返回 true/false；`onLeftClick` 根据返回值维护连续传染计数；只有连续两次点击都返回 true 时触发 `scene5:infected_double`。
- 传染弹窗使用轻量 toast 系统，显示在屏幕左下角，3s 后自动消失。

**相关文件**

- 场景逻辑：`js/scenes/scene5_date.js`
- 关卡数据：`data/scene5_levels.json`（schema: id, title, tip, rows, cols, mineCounts, disabled）
- 关卡编辑器：`editor/scene5_levels_editor.py`（PyQt6，不保存 `mines` 字段）
- 成就：`js/core/achievements.js`（包含 5-0..5-5 等相关注册）

**测试建议**

- 验证传染：连续两次点击且两次均导致传染，应解锁成就 5-5（或在事件队列看到 `scene5:infected_double`）。
- 验证 toast：传染发生时左下角应弹出提示并自动消失。
- 验证禁用格：`disabled` 列表中格子应不可交互且不参与邻居计数。
...
| `5-0`   | 通关第五幕       | 完成了第五幕（心跳扫雷），进入下一段记忆。                             | true               |
| `5-1`   | 扫雷高手         | 以最高难度完成所有关卡。                                               | true               |
| `5-2`   | 拆弹专家         | 从未踩雷完成所有关卡。                                                 | false              |
| `5-3`   | 一发入魂         | 某个关卡第一次点击就踩雷。                                             | false              |
| `5-4`   | 上上下下左右左右 | 触发作弊按钮。                                                         | false              |
| `5-5`   | 病原体           | 连续两次有效左键点击均触发传染（触发 `scene5:infected_double` 事件）。 | false              |
